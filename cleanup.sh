...
for id in $(echo "$RUNNER_LIST" | jq -r '.[] | @base64'); do
        ...
        echo $(_jq '.id')
        curl -X DELETE -H "Accept: application/vnd.github+json" -H "Authorization: Bearer ${TOKEN}" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/orgs/ORG/actions/runners/$(_jq '.id')
done
